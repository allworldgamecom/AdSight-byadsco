      ...
      const text =
        media.length === 0
          ...
          : media
              .map(
                (m) =>
                  `• ${m.id} — ${m.media_type ?? "UNKNOWN"} — ${m.caption ? m.caption.substring(0, 80) + (m.caption.length > 80 ? "..." : "") : "No caption"} — ${m.timestamp ?? ""}`,
              )
              .join("\n");
